...
SL.out_6000_nonrct <- readRDS(paste0("experts_pred/", score_name, "non_RCT/", type, "_6000.rds"))
results_12000_nonrct <- readRDS(paste0("experts_pred/", score_name, "non_RCT/", "plot_results_", type, "_12000", ".rds"))
SL.out_12000_nonrct <- readRDS(paste0("experts_pred/", score_name, "non_RCT/", type, "_12000.rds"))
results_18000_nonrct <- readRDS(paste0("experts_pred/", score_name, "non_RCT/", "plot_results_", type, "_18000", ".rds"))
SL.out_18000_nonrct <- readRDS(paste0("experts_pred/", score_name, "non_RCT/", type, "_18000.rds"))
...
mean_width_data_6000_nonrct <- dplyr::bind_rows(
  make_smaller_block(1, "Estimated labels", results_6000_nonrct[["mean_width"]], random_rate) %>% group_by(mechanism,type)%>% mutate(level=(row_number()-1)/(length(alphas)-1)) %>% ungroup(), 
  data.frame(
...
mean_width_data_12000_nonrct <- dplyr::bind_rows(
  make_smaller_block(1, "Estimated labels", results_12000_nonrct[["mean_width"]], random_rate) %>% group_by(mechanism,type)%>% mutate(level=(row_number()-1)/(length(alphas)-1)) %>% ungroup(), 
  data.frame(
...
mean_width_data_18000_nonrct <- dplyr::bind_rows(
  make_smaller_block(1, "Estimated labels", results_18000_nonrct[["mean_width"]], random_rate) %>% group_by(mechanism,type)%>% mutate(level=(row_number()-1)/(length(alphas)-1)) %>% ungroup(), 
  data.frame(
...
cov_data_6000_nonrct <- dplyr::bind_rows(
  make_smaller_block(1, "Estimated labels", results_6000_nonrct[["cov_unif"]], random_rate)%>% group_by(mechanism,type)%>% mutate(level=(row_number()-1)/(length(alphas)-1)) %>% ungroup(), 
  map_dfr(1:dim(results_6000_nonrct[["cov_unif"]])[1], function(a) {
    data.frame(
      value = results_6000_nonrct[["cov_unif"]][a, 2, 1],
      mechanism = "GLB",
      type = paste0(random_rate[1])
    )})%>% group_by(mechanism,type)%>% mutate(level=(row_number()-1)/(length(alphas)-1)) %>% ungroup() , 
  map_dfr(1:dim(results_6000_nonrct[["cov_unif"]])[1], function(a) {
    data.frame(
...
cov_data_12000_nonrct <- dplyr::bind_rows(
  make_smaller_block(1, "Estimated labels", results_12000_nonrct[["cov_unif"]], random_rate)%>% group_by(mechanism,type)%>% mutate(level=(row_number()-1)/(length(alphas)-1)) %>% ungroup(), 
...
cov_data_18000_nonrct <- dplyr::bind_rows(
  make_smaller_block(1, "Estimated labels", results_18000_nonrct[["cov_unif"]], random_rate)%>% group_by(mechanism,type)%>% mutate(level=(row_number()-1)/(length(alphas)-1)) %>% ungroup(),  
