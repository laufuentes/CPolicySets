...
# # remove for GIF
# gganimate::anim_save(
#   paste0("inst/images/experts/", folder, "margin/, density_", type, ".gif"),
...
results <- list(mean_cardinality= mean_cardinality, spv_y_random=spv_y_random,
                spv_xi_random = spv_xi_random, spv_y_min = spv_y_min,
                spv_xi_min = spv_xi_min, heatmaps_r=heatmaps_r)
# Save metrics
saveRDS(object = results,
        file = paste0("inst/predictions/plot_results_", type, ".rds"))
...
spv_Y_xi_plot <- ggplot2::ggplot(spv_data %>% filter(level==level_choice),
                            ggplot2::aes(x = value_Y,
                                         ...
                                         color = color_group,
                                         group=color_group)) +
  ggplot2::geom_point(aes(shape=choice), size=2) +
...
# rows: observations
# columns: treatment levels
names_experts <- c("conformal-set-valued-policy-learning", "GLB")
